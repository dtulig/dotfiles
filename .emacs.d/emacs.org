...
Keybind for loading the emacs.org file.

#+begin_src emacs-lisp
  (defun dt/open-emacs-org ()
    (interactive)
    (find-file "~/.emacs.d/emacs.org"))

  (global-set-key (kbd "C-c e") #'dt/open-emacs-org)
#+end_src
...
#+begin_src emacs-lisp
  (use-package catppuccin-theme
    ...
    (defun my/set-theme-faces (&optional _frame)
      "Set faces that need resolved theme colors."
    ...
    (defun my/apply-catppuccin-to-frame (frame)
      "Reapply Catppuccin on new terminal frames (emacsclient daemon fix)."
    ...
    (with-eval-after-load 'corfu (my/set-theme-faces))
    (add-hook 'after-make-frame-functions #'my/apply-catppuccin-to-frame))
#+end_src
...
#+begin_src emacs-lisp
  (use-package gptel
    ...
    (setq
     ...
     gptel-model 'qwen36-35b-a3b
     gptel-backend (gptel-make-openai "llama-cpp"
                     :host dt/gptel-host
                     :stream t
                     :protocol "http"
                     :models '(gemma-31b qwen35-27b qwen36-35b-a3b qwen35-122b-a10b))))
#+end_src

#+begin_src emacs-lisp
  (use-package dt-commit
    :demand t
    :load-path "lisp/"
    ;:hook (git-commit-setup . dt-commit-generate-message)
    :bind (:map git-commit-mode-map
                ("M-g" . dt-commit-generate-message)))
#+end_src
...
#+begin_src emacs-lisp
  (use-package aider
    :ensure t
    :config
    (setq aider-args '("--model" "openai/gpt-oss-120b" "--no-auto-accept-architect" "--timeout" "3600" "--map-tokens" "4096" "--editor-model" "openai/qwen3-30b-coder"
  "--no-auto-commit" "--edit-format" "diff" "--editor-edit-format" "diff"))
    ;; (setq aider-args '("--model" "openai/qwen3-30b-instruct" "--editor-model"
  ;; "openai/qwen3-30b-coder" "--no-auto-accept-architect" "--timeout" "3600" "--map-tokens" "2048"
  ;; "--no-auto-commit" "--edit-format" "diff" "--editor-edit-format" "diff"))
    (setenv "OPENAI_API_BASE" (concat "http://" dt/gptel-host "/v1"))
    (setenv "OPENAI_API_KEY" "local")
    (global-set-key (kbd "C-c g") 'aider-transient-menu))
#+end_src

* Org Mode
** Variables and Paths
#+begin_src emacs-lisp
  (defvar dt/org-directory (expand-file-name "~/Sync/org/"))
  (defvar dt/org-projects-dir (expand-file-name  (concat dt/org-directory "projects/")))

  (defun dt/org--expand (path)
    (expand-file-name path dt/org-directory))
#+end_src

** Utility and project support
#+begin_src emacs-lisp
  (defun dt/create-org-path (path)
    "Create an Org filename in `dt/org-projects-dir` from PATH."
    (let* ((trimmed (if (equal (substring path -1) "/")
                        (substring path 0 -1)
                      path))
           (last (car (last (split-string trimmed "/")))))
      (concat dt/org-projects-dir
              (downcase
  ...
  (defun dt/project-org-file ()
    "Return the Org file path for the current project."
    (dt/create-org-path (cdr (project-current))))

  (defun dt/switch-to-project-org-file ()
    "Open the Org file for the current project."
    (interactive)
    (find-file (dt/project-org-file)))

  (defun dt/org-reload ()
    "Reload agenda files and defaults based on current time."
    (interactive)
    (setq org-agenda-files
          (append
           (file-expand-wildcards (dt/org--expand "dates/*.org"))
           (file-expand-wildcards (dt/org--expand "*.org"))
           (file-expand-wildcards (dt/org--expand "projects/*.org"))
           (file-expand-wildcards (dt/org--expand "steadily/dates/*.org"))
           (file-expand-wildcards (dt/org--expand "steadily/*.org"))
           (file-expand-wildcards (dt/org--expand "indeed/dates/*.org"))
           (file-expand-wildcards (dt/org--expand "indeed/*.org"))))
    (setq org-default-personal-notes-file
          (concat dt/org-directory "dates/"
                  (downcase (format-time-string "%Y-%B-p.org"))))
    (setq org-default-work-notes-file
          (concat dt/org-directory "indeed/dates/"
                  (downcase (format-time-string "%Y-%B-w.org"))))
    (setq org-default-steadily-notes-file
          (concat dt/org-directory "steadily/dates/"
                  (downcase (format-time-string "%Y-%B-w.org"))))
    (setq org-default-journal-file
          (concat dt/org-directory "journal.org"))
    (dt/org-setup-capture-templates))
#+end_src
...
#+begin_src emacs-lisp
  (use-package org
    ...
    :custom
    (org-directory dt/org-directory)
    (org-log-done 'time)
    (org-default-notes-file (dt/org--expand "notes.org"))
    ...
    (org-todo-keyword-faces
     '(("TODO" :inherit error :weight bold)
       ("NEXT" :inherit font-lock-keyword-face :weight bold)
       ("DONE" :inherit success :weight bold)
       ("WAITING" :inherit warning :weight bold)
       ("DELEGATED" :inherit warning :weight bold)
       ("HOLD" :inherit font-lock-constant-face :weight bold)
       ("CANCELLED" :inherit success :weight bold)
       ("MEETING" :inherit success :weight bold)
       ("PHONE" :inherit success :weight bold)))
...
#+begin_src emacs-lisp
  (defun dt/org-setup-capture-templates ()
    "Set up org-capture templates after org-reload has set the file variables."
    (setq org-capture-templates
          `(("t" "Tasks")
            ("ts" "Steadily" entry
             (file+headline ,org-default-steadily-notes-file "Tasks")
             "* TODO %^{Description}\n%U\n%?"
             :clock-in t :clock-resume t :empty-lines 1)
            ("tp" "Personal" entry
             (file+headline ,org-default-personal-notes-file "Tasks")
             "* TODO %^{Description}\n%U\n%?"
             :clock-in t :clock-resume t)

            ("m" "Meeting")
            ("ms" "Steadily" entry
             (file+headline ,org-default-steadily-notes-file "Meetings")
             "* MEETING with %^{Description} :MEETING:\n%U\n%?"
             :clock-in t :clock-resume t)
            ("mp" "Personal" entry
             (file+headline ,org-default-personal-notes-file "Meetings")
             "* MEETING with %^{Description} :MEETING:\n%U\n%?"
             :clock-in t :clock-resume t)
            ...
            ("n" "Note")
            ("ns" "Steadily" entry
             (file+headline ,org-default-steadily-notes-file "Notes")
             "* %? :NOTE:\n%U"
             :clock-in t :clock-resume t)
            ("np" "Personal" entry
             (file+headline ,org-default-personal-notes-file "Notes")
             "* %? :NOTE:\n%U"
             :clock-in t :clock-resume t)

            ("P" "Todo (Project)" entry
             (file+headline dt/project-org-file "Tasks")
             "* TODO %^{Description}\n%U\n%?"
             :clock-in t :clock-resume t)
            ("N" "Project Note" entry
             (file+headline dt/project-org-file "Notes")
             "* %?\n%U")
            ...
            ("j" "Journal" entry
             ...
             :clock-in t :clock-resume t)
            ("l" "Log Time" entry
             (file+olp+datetree ,(dt/org--expand "timelog.org"))
             "** %U - %^{Activity}  :TIME:"))))
#+end_src

** Org contrib, babel, and binding
#+begin_src emacs-lisp
  (use-package org-contrib
    ...
    :bind (("C-c s" . org-store-link)
           ("C-c c" . org-capture)
           ("C-c a" . org-agenda))
    :hook ((midnight-mode . dt/org-reload))
    :config
...
#+begin_src emacs-lisp
  ...
  (defun dt/buffer-org-mode-p (buffer)
  ...
  (defun dt/org-close-all-org-buffers ()
    (interactive)
    (mapc #'kill-buffer
          (cl-remove-if-not #'dt/buffer-org-mode-p (buffer-list))))

  (defun dt/org-agenda-redo-with-close-buffers ()
    (interactive)
    (dt/org-close-all-org-buffers)
    (org-agenda-redo t))

  (defun dt/org-ql-project-todo-excludes ()
    '(and (todo)
          (not (todo "SOMEDAY" "CANCELLED" "DELEGATED" "DONE"))
          (not (tags "SOMEDAY" "HOLD"))))
        ...
        '(("A" "Agenda"
          ...
          ("R" "Weekly Review"
           ((org-ql-block '(and (todo)
            ...
            (org-ql-block '(and (not (descendants (todo "NEXT")))
                                (descendants (todo)))
                          ((org-ql-block-header "Stuck Projects")))))
          ("T" "Triage"
           ((org-ql-block '(and (todo "TODO")
                                ...
                                (not (children))
                                (not (tags "SOMEDAY")))
                          ((org-ql-block-header "Orphan TODOs (not in a project, no children)")))
            (org-ql-block '(and (todo "TODO")
                                (not (ancestors (todo)))
                                (not (children))
                                (not (tags "SOMEDAY"))
                                (ts :to -14))
                          ((org-ql-block-header "Stale atomic TODOs (> 2 weeks, should refile or decide)")))
            (org-ql-block '(and (todo "TODO")
                                (descendants (todo))
                                (not (descendants (todo "NEXT")))
                                (not (tags "SOMEDAY" "HOLD")))
                          ((org-ql-block-header "Projects missing a NEXT (promote a child)")))))))

  ;; Add category formatting advice
  (defun dt/org-ql-view--format-element (orig-fun element)
    (if (not element)
        ""
      (let* ((properties (cadr element))
             (result (funcall orig-fun element))
             (marker (plist-get properties :org-marker))
             (category (when marker
                         (org-entry-get marker "CATEGORY" t)))
             (cat-str (if category
                          (truncate-string-to-width (concat category ":") 15 nil ?\s)
                        (make-string 15 ?\s))))
        (concat "  " cat-str " " result))))
#+end_src
...
#+begin_src emacs-lisp
  (use-package org-habit
    ...
    (defun my/org-habit-set-faces (&optional _frame)
      (let ((bg (face-background 'default))
            (surface (catppuccin-get-color 'surface1)))
        ...
        (set-face-attribute 'org-habit-ready-face nil
                            :background (face-foreground 'success)
                            :foreground bg)
    ...
    (my/org-habit-set-faces)
    (add-hook 'after-make-frame-functions #'my/org-habit-set-faces)
...
#+begin_src emacs-lisp
  ...
  (use-package evil-org-agenda
    ...
    (evil-define-key 'motion org-agenda-mode-map
      ...
      "gl" #'dt/org-agenda-redo-with-close-buffers))
...
#+begin_src emacs-lisp
  (use-package denote
    ...
    (("C-c n n" . denote)
     ...
     ("C-c n g" . denote-grep))
    :config
    (setq denote-directory (expand-file-name "~/Sync/org/zet/"))
...
#+begin_src emacs-lisp
  (use-package eglot
    ...
    (add-to-list 'eglot-server-programs
                 '((kotlin-ts-mode kotlin-mode) . ("kotlin-lsp.sh" "--stdio")))
...
#+begin_src emacs-lisp
  (use-package web-mode
    ...
    :init
    (define-derived-mode tsx-ts-mode web-mode "TypeScript-tsx")
    :custom
...
#+begin_src emacs-lisp
  (use-package flyspell
    ...
    (defun my/flyspell-set-faces (&optional _frame)
      (set-face-attribute 'flyspell-incorrect nil
                          :underline `(:style wave :color ,(face-foreground 'error)))
      (set-face-attribute 'flyspell-duplicate nil
                          :underline `(:style wave :color ,(face-foreground 'warning))))

    (my/flyspell-set-faces)
    (add-hook 'after-make-frame-functions #'my/flyspell-set-faces))
#+end_src
