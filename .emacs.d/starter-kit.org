...
- Function for loading other parts of the starter kit
  ...
  #+begin_src emacs-lisp
    (defun load-file-or-header-or-tag (prefix file &optional header-or-tag)
      "Load configuration from other file-*.org files.
    If the optional argument is the id of a subtree then only
    configuration from within that subtree will be loaded.  If it is
    not an id then it will be interpreted as a tag, and only subtrees
    marked with the given tag will be loaded."
      (let ((file (expand-file-name (if (string-match (concat prefix "-.+\.org") file)
                                        file
                                      (format (concat prefix "-%s.org") file))
                                    starter-kit-dir)))
        (org-babel-load-file
         (if header-or-tag
    ...
    to your configuration."
      (load-file-or-header-or-tag "starter-kit" file header-or-tag))
  #+end_src

- Function for loading other parts of the init.
  #+begin_src emacs-lisp
    (defun dt-init-load (file &optional header-or-tag)
      (load-file-or-header-or-tag "init" file header-or-tag))
  #+end_src
...
#+begin_src emacs-lisp
  (cl-flet ((sk-load (base)
    ...
    (let ((user-dir (expand-file-name user-login-name starter-kit-dir)))
      ...
      (sk-load (concat "system/" system-name))
